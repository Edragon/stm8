  ...
  20  000e 45            	dc.b	69
  21  000f 46            	dc.b	70
  52                     ; 31 void Init_UART2(void)
  52                     ; 32 {
  54                     	switch	.text
  55  0000               _Init_UART2:
  59                     ; 33 	UART2_DeInit();
  61  0000 cd0000        	call	_UART2_DeInit
  63                     ; 34 	UART2_Init((u32)9600, UART2_WORDLENGTH_8D, UART2_STOPBITS_1, UART2_PARITY_NO, UART2_SYNCMODE_CLOCK_DISABLE, UART2_MODE_TX_ENABLE);
  65  0003 4b04          	push	#4
  ...
  73  0014 89            	pushw	x
  74  0015 cd0000        	call	_UART2_Init
  76  0018 5b09          	addw	sp,#9
  77                     ; 36 }
  80  001a 81            	ret
 116                     ; 37 void Send(uint8_t dat)
 116                     ; 38 {
 117                     	switch	.text
 118  001b               _Send:
 120  001b 88            	push	a
 ...
 124  001c               L14:
 125                     ; 39   while(( UART2_GetFlagStatus(UART2_FLAG_TXE)==RESET));
 127  001c ae0080        	ldw	x,#128
 128  001f cd0000        	call	_UART2_GetFlagStatus
 130  0022 4d            	tnz	a
 131  0023 27f7          	jreq	L14
 132                     ; 41 		UART2_SendData8(dat);
 134  0025 7b01          	ld	a,(OFST+1,sp)
 135  0027 cd0000        	call	_UART2_SendData8
 137                     ; 43 }
 140  002a 84            	pop	a
 141  002b 81            	ret
 168                     ; 44 void Init_ADC(void)
 168                     ; 45 {
 169                     	switch	.text
 170  002c               _Init_ADC:
 174                     ; 46 	GPIO_Init(GPIOD, GPIO_PIN_2, GPIO_MODE_IN_FL_NO_IT);
 ...
 186                     ; 48 ADC1_Init(ADC1_CONVERSIONMODE_SINGLE, ADC1_CHANNEL_3, ADC1_PRESSEL_FCPU_D2, ADC1_EXTTRIG_TIM, DISABLE, ADC1_ALIGN_RIGHT, ADC1_SCHMITTTRIG_CHANNEL3, ENABLE);
 ...
 200                     ; 49 ADC1_Cmd(ENABLE);
 202  0050 a601          	ld	a,#1
 203  0052 cd0000        	call	_ADC1_Cmd
 205                     ; 53 }
 208  0055 81            	ret
 244                     ; 54 void UART2_sendhex(unsigned char dat)
 244                     ; 55 {
 245                     	switch	.text
 246  0056               _UART2_sendhex:
 248  0056 88            	push	a
 ...
 289  0078 84            	pop	a
 290  0079 81            	ret
 326                     ; 62 void UART2_sendstr(unsigned char *dat)
 326                     ; 63 {
 327                     	switch	.text
 328  007a               _UART2_sendstr:
 330  007a 89            	pushw	x
 ...
 436                     ; 75 		Init_UART2();
 438  0092 cd0000        	call	_Init_UART2
 440                     ; 76 		Init_ADC();
 442  0095 ad95          	call	_Init_ADC
 444  0097               L151:
 445                     ; 81 		ADC1_StartConversion();
 447  0097 cd0000        	call	_ADC1_StartConversion
 449                     ; 82     flag_status = ADC1_GetFlagStatus(ADC1_FLAG_EOC);
 451  009a a680          	ld	a,#128
 452  009c cd0000        	call	_ADC1_GetFlagStatus
 454  009f 6b01          	ld	(OFST-2,sp),a
 456  00a1               L161:
 457                     ; 83     while(flag_status == RESET); // SET or RESET
 459  00a1 0d01          	tnz	(OFST-2,sp)
 460  00a3 27fc          	jreq	L161
 461                     ; 84     		u16_adc1_value = ADC1_GetConversionValue();
 463  00a5 cd0000        	call	_ADC1_GetConversionValue
 465  00a8 1f02          	ldw	(OFST-1,sp),x
 466                     ; 85 				UART2_sendhex((u16_adc1_value>>8));
 468  00aa 7b02          	ld	a,(OFST-1,sp)
 469  00ac ada8          	call	_UART2_sendhex
 471                     ; 86 		    UART2_sendhex((u16_adc1_value&0xff));
 473  00ae 7b03          	ld	a,(OFST+0,sp)
 474  00b0 a4ff          	and	a,#255
 475  00b2 ada2          	call	_UART2_sendhex
 477                     ; 87 		    UART2_sendstr("\r\n");
 479  00b4 ae0000        	ldw	x,#L561
 480  00b7 adc1          	call	_UART2_sendstr
 483  00b9 20dc          	jra	L151
 518                     ; 101 void assert_failed(u8* file, u32 line)
 518                     ; 102 { 
 519                     	switch	.text
 ...
 550                     	xdef	_main
 551                     	xdef	_UART2_sendstr
 552                     	xdef	_UART2_sendhex
 553                     	xdef	_Init_ADC
 554                     	xdef	_Send
 555                     	xdef	_Init_UART2
 556                     	xdef	_HexTable
 557                     	xdef	_assert_failed
 558                     	xref	_UART2_GetFlagStatus
 559                     	xref	_UART2_SendData8
 560                     	xref	_UART2_Init
 561                     	xref	_UART2_DeInit
 562                     	xref	_GPIO_Init
