  ...
  /*!<Set High speed internal clock  */
   ...
   while (1)
   {
      ...
      if(UART_RX_NUM&0x80)
      {
        ...
        UART1_SendString("2-You sent the messages is:",sizeof("2-You sent the messages is"));
        UART1_SendString(RxBuffer,len);
        UART1_SendByte('\n');
        //UART1_SendByte(RxBuffer[0]);
        //UART1_SendByte('\n');
...
//        }
        ...
        if (RxBuffer[0] == 65)    // can use 'byte', hex 0x41 or 65 to read first byte "A"
        {
          LED1(ON);
          Delay(0x2ffff);
          LED1(OFF);
